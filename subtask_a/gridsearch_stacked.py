...
def stacked_lstm_model(hidden_dim):
    ...
    hidden = Bidirectional(LSTM(hidden_dim // 2, recurrent_dropout=0.2, return_sequences=True))(embedded)
    hidden = Bidirectional(LSTM(hidden_dim // 2, recurrent_dropout=0.2))(hidden)
...
if __name__ == '__main__':
    ...
    model = KerasClassifier(build_fn = stacked_lstm_model,batch_size = 128,nb_epoch = 20 ,verbose = 1)
    # define the grid search parameters
    # batch_size = [8,16,32,64,100,128]
    # epochs = [20]
    # param_grid = dict(batch_size=batch_size, epochs=epochs)
    # r_dropout = [0.25, 0.3, 0.35, 0.4, 0.45, 0.5, 0.55, 0.6, 0.65, 0.7]
    # param_grid = dict(r_dropout=r_dropout)
    hidden_dim = [40, 60, 80, 100, 120, 140, 160]
